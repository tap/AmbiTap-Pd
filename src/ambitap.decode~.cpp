...
static std::vector<ambitap::spherical_coord> layout_from_name(const char* name) {
    ...
    if (!std::strcmp(name, "stereo")) return stereo();
    if (!std::strcmp(name, "quad")) return quad();
    if (!std::strcmp(name, "surround_5_1") || !std::strcmp(name, "5.1")) return surround_5_1();
    if (!std::strcmp(name, "surround_7_1") || !std::strcmp(name, "7.1")) return surround_7_1();
    if (!std::strcmp(name, "surround_7_1_4") || !std::strcmp(name, "7.1.4")) return surround_7_1_4();
    if (!std::strcmp(name, "cube")) return cube();
    if (!std::strcmp(name, "hexagon")) return hexagon();
    if (!std::strcmp(name, "octagon")) return octagon();
    return {};
}
...
static t_int* decode_perform(t_int* w) {
    ...
    p->dec.process(p->in_ptrs.data(), p->out_ptrs.data(),
                   static_cast<size_t>(p->spk), static_cast<size_t>(n));
    return w + 6;
}
...
static void decode_dsp(t_ambitap_decode_tilde* x, t_signal** sp) {
    ...
    dsp_add(decode_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
...
static void decode_decoder_type(t_ambitap_decode_tilde* x, t_symbol* s) {
    ...
    if (!std::strcmp(s->s_name, "allrad")) x->p->dec.set_algorithm(alg::allrad);
    else if (!std::strcmp(s->s_name, "epad")) x->p->dec.set_algorithm(alg::epad);
    else x->p->dec.set_algorithm(alg::mode_match);
}
static void decode_max_re(t_ambitap_decode_tilde* x, t_floatarg f) { x->p->dec.set_max_re(f != 0); }
...
static void* decode_new(t_symbol*, int argc, t_atom* argv) {
    ...
    if (speakers.empty()) speakers = ambitap::layouts::stereo();
...
static void decode_free(t_ambitap_decode_tilde* x) { delete x->p; }

void ambitap_decode_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.decode~"),
                           reinterpret_cast<t_newmethod>(decode_new),
                           reinterpret_cast<t_method>(decode_free),
                           sizeof(t_ambitap_decode_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
