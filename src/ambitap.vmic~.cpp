...
struct vmic_impl {
    ...
    explicit vmic_impl(int order)
        : mic(order), ch(static_cast<int>(mic.channels())), in_ptrs(static_cast<size_t>(ch)) {}
};
...
static void vmic_dsp(t_ambitap_vmic_tilde* x, t_signal** sp) {
    ...
    dsp_add(vmic_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
            sp[1]->s_vec, static_cast<t_int>(sp[0]->s_length));
}

static void vmic_azimuth(t_ambitap_vmic_tilde* x, t_floatarg f) { x->p->mic.set_azimuth(static_cast<float>(f)); }
static void vmic_elevation(t_ambitap_vmic_tilde* x, t_floatarg f) { x->p->mic.set_elevation(static_cast<float>(f)); }
static void vmic_max_re(t_ambitap_vmic_tilde* x, t_floatarg f) { x->p->mic.set_max_re(f != 0); }
...
static void vmic_free(t_ambitap_vmic_tilde* x) { delete x->p; }

void ambitap_vmic_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.vmic~"),
                           reinterpret_cast<t_newmethod>(vmic_new),
                           reinterpret_cast<t_method>(vmic_free),
                           sizeof(t_ambitap_vmic_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
