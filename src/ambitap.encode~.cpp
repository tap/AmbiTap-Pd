...
static void ambitap_encode_tilde_dsp(t_ambitap_encode_tilde* x, t_signal** sp) {
    ...
    dsp_add(ambitap_encode_tilde_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec,
...
static void* ambitap_encode_tilde_new(t_symbol* s, int argc, t_atom* argv) {
    (void)s;
    t_ambitap_encode_tilde* x =
        reinterpret_cast<t_ambitap_encode_tilde*>(pd_new(ambitap_encode_tilde_class));
    int order = 1;
    if (argc >= 1)
        order = static_cast<int>(atom_getfloat(argv));
    order        = std::clamp(order, 0, ambitap::max_order);
...
void ambitap_encode_tilde_setup(void) {
    ambitap_encode_tilde_class = class_new(
        gensym("ambitap.encode~"),
        reinterpret_cast<t_newmethod>(ambitap_encode_tilde_new),
        reinterpret_cast<t_method>(ambitap_encode_tilde_free),
        sizeof(t_ambitap_encode_tilde),
        CLASS_MULTICHANNEL,
        A_GIMME, 0);

    CLASS_MAINSIGNALIN(ambitap_encode_tilde_class, t_ambitap_encode_tilde, x_f);
    class_addmethod(ambitap_encode_tilde_class,
                    reinterpret_cast<t_method>(ambitap_encode_tilde_dsp),
                    gensym("dsp"), A_CANT, 0);
    class_addmethod(ambitap_encode_tilde_class,
                    reinterpret_cast<t_method>(ambitap_encode_tilde_azimuth),
                    gensym("azimuth"), A_FLOAT, 0);
    class_addmethod(ambitap_encode_tilde_class,
                    reinterpret_cast<t_method>(ambitap_encode_tilde_elevation),
                    gensym("elevation"), A_FLOAT, 0);
    class_addmethod(ambitap_encode_tilde_class,
                    reinterpret_cast<t_method>(ambitap_encode_tilde_gain),
