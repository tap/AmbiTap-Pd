...
static void format_dsp(t_ambitap_format_tilde* x, t_signal** sp) {
    ...
    dsp_add(format_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
...
static void format_direction(t_ambitap_format_tilde* x, t_symbol* s) {
    ...
    x->io->proc.set_direction(std::strcmp(s->s_name, "fuma_to_ambix") == 0 ? dir::fuma_to_ambix
                                                                           : dir::ambix_to_fuma);
}
...
static void format_free(t_ambitap_format_tilde* x) { delete x->io; }

void ambitap_format_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.format~"),
                           reinterpret_cast<t_newmethod>(format_new),
                           reinterpret_cast<t_method>(format_free),
                           sizeof(t_ambitap_format_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
