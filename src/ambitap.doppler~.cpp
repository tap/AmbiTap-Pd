...
static void doppler_dsp(t_ambitap_doppler_tilde* x, t_signal** sp) {
    ...
    dsp_add(doppler_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
            sp[1]->s_vec, static_cast<t_int>(sp[0]->s_length));
}

static void doppler_distance(t_ambitap_doppler_tilde* x, t_floatarg f) { x->io->proc.set_distance(static_cast<float>(f)); }
static void doppler_speed(t_ambitap_doppler_tilde* x, t_floatarg f) { x->io->proc.set_speed_of_sound(static_cast<float>(f)); }
static void doppler_maxdist(t_ambitap_doppler_tilde* x, t_floatarg f) { x->io->proc.set_max_distance(static_cast<float>(f)); }
...
static void doppler_free(t_ambitap_doppler_tilde* x) { delete x->io; }

void ambitap_doppler_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.doppler~"),
                           reinterpret_cast<t_newmethod>(doppler_new),
                           reinterpret_cast<t_method>(doppler_free),
                           sizeof(t_ambitap_doppler_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
