...
static void rotate_dsp(t_ambitap_rotate_tilde* x, t_signal** sp) {
    ...
    dsp_add(rotate_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
            sp[1]->s_vec, static_cast<t_int>(sp[0]->s_length));
}

static void rotate_yaw(t_ambitap_rotate_tilde* x, t_floatarg f) { x->io->proc.set_yaw(static_cast<float>(f)); }
static void rotate_pitch(t_ambitap_rotate_tilde* x, t_floatarg f) { x->io->proc.set_pitch(static_cast<float>(f)); }
static void rotate_roll(t_ambitap_rotate_tilde* x, t_floatarg f) { x->io->proc.set_roll(static_cast<float>(f)); }
...
static void rotate_free(t_ambitap_rotate_tilde* x) { delete x->io; }

void ambitap_rotate_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.rotate~"),
                           reinterpret_cast<t_newmethod>(rotate_new),
                           reinterpret_cast<t_method>(rotate_free),
                           sizeof(t_ambitap_rotate_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
