...
static void mirror_dsp(t_ambitap_mirror_tilde* x, t_signal** sp) {
    ...
    dsp_add(mirror_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
            sp[1]->s_vec, static_cast<t_int>(sp[0]->s_length));
}

static void mirror_flip_lr(t_ambitap_mirror_tilde* x, t_floatarg f) { x->io->proc.set_flip_lr(f != 0); }
static void mirror_flip_fb(t_ambitap_mirror_tilde* x, t_floatarg f) { x->io->proc.set_flip_fb(f != 0); }
static void mirror_flip_ud(t_ambitap_mirror_tilde* x, t_floatarg f) { x->io->proc.set_flip_ud(f != 0); }
...
static void mirror_free(t_ambitap_mirror_tilde* x) { delete x->io; }

void ambitap_mirror_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.mirror~"),
                           reinterpret_cast<t_newmethod>(mirror_new),
                           reinterpret_cast<t_method>(mirror_free),
                           sizeof(t_ambitap_mirror_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
