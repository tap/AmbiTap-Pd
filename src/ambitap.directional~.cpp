...
static void directional_dsp(t_ambitap_directional_tilde* x, t_signal** sp) {
    ...
    dsp_add(directional_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
            sp[1]->s_vec, static_cast<t_int>(sp[0]->s_length));
}

static void directional_azimuth(t_ambitap_directional_tilde* x, t_floatarg f) { x->io->proc.set_azimuth(static_cast<float>(f)); }
static void directional_elevation(t_ambitap_directional_tilde* x, t_floatarg f) { x->io->proc.set_elevation(static_cast<float>(f)); }
static void directional_gain(t_ambitap_directional_tilde* x, t_floatarg f) { x->io->proc.set_gain(static_cast<float>(f)); }
...
static void directional_free(t_ambitap_directional_tilde* x) { delete x->io; }

void ambitap_directional_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.directional~"),
                           reinterpret_cast<t_newmethod>(directional_new),
                           reinterpret_cast<t_method>(directional_free),
                           sizeof(t_ambitap_directional_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
