...
struct bed2hoa_impl {
    ...
    bed2hoa_impl(int order, const std::vector<ambitap::spherical_coord>& speakers)
        ...
        gains.assign(static_cast<size_t>(spk),
                     std::vector<double>(static_cast<size_t>(hoa_ch), 0.0));
...
static t_int* bed2hoa_perform(t_int* w) {
    ...
    for (int ch = 0; ch < p->hoa_ch; ++ch) {
        ...
        for (int s = 0; s < in_ch; ++s) {
            const float     gs = static_cast<float>(p->gains[static_cast<size_t>(s)]
                                                     [static_cast<size_t>(ch)]) * g;
...
static void bed2hoa_dsp(t_ambitap_bed2hoa_tilde* x, t_signal** sp) {
    ...
    dsp_add(bed2hoa_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans),
...
static void bed2hoa_free(t_ambitap_bed2hoa_tilde* x) { delete x->p; }

void ambitap_bed2hoa_tilde_setup(void) {
    t_class* c = class_new(gensym("ambitap.bed2hoa~"),
                           reinterpret_cast<t_newmethod>(bed2hoa_new),
                           reinterpret_cast<t_method>(bed2hoa_free),
                           sizeof(t_ambitap_bed2hoa_tilde), CLASS_MULTICHANNEL, A_GIMME, 0);
