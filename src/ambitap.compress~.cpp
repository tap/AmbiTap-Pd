...
static void compress_dsp(t_ambitap_compress_tilde* x, t_signal** sp) {
    ...
    dsp_add(compress_perform, 5, x, sp[0]->s_vec, static_cast<t_int>(sp[0]->s_nchans), sp[1]->s_vec,
            static_cast<t_int>(sp[0]->s_length));
}

static void compress_threshold(t_ambitap_compress_tilde* x, t_floatarg f) {
    x->io->proc.set_threshold_db(static_cast<float>(f));
}
static void compress_ratio(t_ambitap_compress_tilde* x, t_floatarg f) {
    x->io->proc.set_ratio(static_cast<float>(f));
}
static void compress_attack(t_ambitap_compress_tilde* x, t_floatarg f) {
    x->io->proc.set_attack(static_cast<float>(f));
}
static void compress_release(t_ambitap_compress_tilde* x, t_floatarg f) {
    x->io->proc.set_release(static_cast<float>(f));
}
static void compress_makeup(t_ambitap_compress_tilde* x, t_floatarg f) {
    x->io->proc.set_makeup_gain_db(static_cast<float>(f));
}
...
static void compress_free(t_ambitap_compress_tilde* x) {
    delete x->io;
}

void ambitap_compress_tilde_setup(void) {
    t_class* c                   = class_new(gensym("ambitap.compress~"), reinterpret_cast<t_newmethod>(compress_new),
                                             reinterpret_cast<t_method>(compress_free), sizeof(t_ambitap_compress_tilde),
