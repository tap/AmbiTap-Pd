...
def gen_encode():
    ...
    az = live_ctrl(p, 340, 120, "azimuth", "azimuth (rad \\, pi/2 = left)")
    el = p.msg(340, 200, "elevation 0.5")
...
def gen_rotate():
    ...
    header(p, "ambitap.rotate~ <order>",
           "Rotate a HOA bus by yaw / pitch / roll (Euler Z-Y-X \\, radians).")
    src = chain_source(p)
    ...
    p.connect(bin, 0, dac, 0)
    p.connect(bin, 1, dac, 1)
    yaw = live_ctrl(p, 340, 120, "yaw", "yaw (rad)")
...
def gen_binaural():
    ...
    p.connect(bin, 0, dac, 0)
    p.connect(bin, 1, dac, 1)
    yaw = live_ctrl(p, 340, 120, "yaw", "head yaw (rad)")
    pit = p.msg(340, 200, "pitch 0")
    rol = p.msg(340, 240, "roll 0")
...
def gen_vmic():
    ...
    az = live_ctrl(p, 340, 120, "azimuth", "look azimuth (rad)")
